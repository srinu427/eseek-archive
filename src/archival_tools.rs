...
pub struct LocalArchiveReader<T: Read + Seek>{
  fr: T,
  entries: HashMap<String, ArchiveEntryDetails>
}

impl<T: Read + Seek> LocalArchiveReader<T>{
  pub fn new(mut fr: T) -> Result<Self, String>{
    ...
    let entries = ArchiveEntryDetails::hashmap_from_appendix(appendix)?;
    Ok(Self{
      fr,
      entries
    })
  ...
  pub fn extract_file(
    ...
    self.entries
      ...
      .extract_to_directory(&mut self.fr, out_dir)?;
    Ok(())
  }

  pub fn extract_all(&mut self, out_dir: &Path) -> Result<(), String>{
    for (_, entry) in self.entries.iter(){
      entry.extract_to_directory(&mut self.fr, out_dir)?;
    }
    Ok(())
  }
}
...
pub struct S3ArchiveReader{
  ...
  bucket_name: String,
  object_name: String,
  file_list: HashMap<String, ArchiveEntryDetails>,
}
...
impl S3ArchiveReader{
  pub async fn new(
    ...
    let appendix_desc = s3_client
      ...
      .send()
      .await
      .map_err(|e| format!("error getting object from S3: {e}"))?
      .body
      .collect()
      .await
      .map(|x| x.into_bytes())
      .map_err(|e| format!("error S3 object content: {e}"))?;
    let mut appendix_offset_bytes = [0u8; 8];
    appendix_offset_bytes.copy_from_slice(&appendix_desc[0..8]);
    let mut appendix_size_bytes = [0u8; 8];
    appendix_size_bytes.copy_from_slice(&appendix_desc[8..16]);
    let appendix_offset = u64::from_le_bytes(appendix_offset_bytes);
    let appendix_size = u64::from_le_bytes(appendix_size_bytes);
    let compressed_appendix = s3_client
      .get_object()
      .bucket(bucket_name)
      .key(object_name)
      .range(format!(
        "bytes={appendix_offset}-{}",
        appendix_offset + appendix_size - 1
      ))
      .send()
      .await
      .map_err(|e| format!("error getting object from S3: {e}"))?
